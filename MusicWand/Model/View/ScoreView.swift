...
struct ScoreView: View {
    @ObservedObject var scoreGame: ScoreGame
            ...
            VStack {
                ZStack {
                    GeometryReader { geo in
                        ...
                        ForEach(Array(self.scoreGame.notes), id: \.self) { note in
                            Image(note.imgName)
                            .resizable()
                            ...
                            .gesture(DragGesture().onChanged({ value in
                                ...
                                if self.fromPoint == nil {
                                    ...
                                    self.movingNote = self.scoreGame.noteAt(col: fromCol, row: fromRow)
                                }
                            }).onEnded({ value in
    ...
    func xyToColRow(bounds: CGRect, x: CGFloat, y: CGFloat) -> (Int, Int) {
        let cellSide = cellWidth(bounds: bounds) + cellHeight(bounds: bounds) + 0.5
        let col: Int = Int((x - originX(bounds: bounds)) / cellSide)
        let row: Int = Int((y - originY(bounds: bounds)) / cellSide)
        return (col, row)
    }
